...
def grab_train_img_from_names(train_dataset_processed, train_dataset_transform):
  train_dataset = []
...
def grab_test_img_from_names(test_dataset_processed, test_dataset_transform):
  test_dataset = []
  for i in tqdm(range(len(test_dataset_processed))):
    input = cv2.imread(test_dataset_transform[i])
    real = cv2.imread(test_dataset_processed[i])
    input, real = resize(input, real, 512, 512)
    input, real = normalize(input, real)
    test_dataset.append(np.array([input, real]))
  test_dataset = np.array(train_dataset)
  return test_dataset
...
def generate_images(model, test_input, tar):
  ...
  for i in range(3):
    ...
    try:
        ...
        pass
    plt.imsave(r'C:\Users\Athrva Pandhare\Desktop\New folder (4)\Samples\{}_{}.jpg'.format(sample_num, i),img) 
    plt.axis('off')
    plt.close()
  #plt.show()
